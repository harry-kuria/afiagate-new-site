...
const ProfilePage: React.FC = () => {
  ...
  const loadProfileData = useCallback(async () => {
    ...
    try {
      ...
      // Load profile data
      const profileData = await connectApiService.getProfile(user.id);
      setProfileUser(profileData);

      // Load reviews
      const reviewsData = await connectApiService.getReviews(user.id);
      setReviews(reviewsData.data);

      // Load education
      const educationData = await connectApiService.getEducation(user.id);
      setEducation(educationData.data);

      // Load licensure
      const licensureData = await connectApiService.getLicensure(user.id);
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <CardContent>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, flexWrap: 'wrap' }}>
            <Avatar
              ...
              src={`https://ui-avatars.com/api/?name=${profileUser.full_name}&size=120&background=random`}
            />
            <Box sx={{ flex: 1, minWidth: 300 }}>
              ...
              <Typography variant="h6" color="text.secondary" gutterBottom>
                {profileUser.specialization || 'Healthcare Professional'}
              </Typography>
              ...
              <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap' }}>
                <Chip
                  icon={<LocationOn />}
                  label={profileUser.location || 'Location not specified'}
                  variant="outlined"
                />
                <Chip
                  icon={<Work />}
                  label={profileUser.experience || 'Experience not specified'}
                  variant="outlined"
                />
                <Chip
                  icon={profileUser.is_available ? <CheckCircle /> : <Schedule />}
                  label={profileUser.is_available ? 'Available' : 'Not Available'}
                  color={profileUser.is_available ? 'success' : 'default'}
                  variant="outlined"
                />
              </Box>
              ...
              <Paper sx={{ p: 2 }}>
                ...
                <List>
                  <ListItem>
                    ...
                    <ListItemText 
                      primary="Full Name" 
                      secondary={profileUser.full_name} 
                    />
                  </ListItem>
                  <ListItem>
                  ...
                  <ListItem>
                    ...
                    <ListItemText 
                      primary="Phone" 
                      secondary={profileUser.phone_number || 'Not provided'} 
                    />
                  </ListItem>
                  <ListItem>
                    ...
                    <ListItemText 
                      ...
                      secondary={profileUser.specialization || 'Not specified'} 
                    />
                  </ListItem>
                </List>
              </Paper>
          ...
          <Box sx={{ display: 'flex', gap: 2, flexDirection: { xs: 'column', sm: 'row' } }}>
            ...
            <Box sx={{ flex: 1 }}>
              <Paper sx={{ p: 2 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  {profileUser.phone_number || 'Not provided'}
                </Typography>
              </Paper>
            </Box>
          </Box>
      ...
      <Dialog open={contactDialogOpen} onClose={() => setContactDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Contact Harry</DialogTitle>
        <DialogContent>
          <TextField
            select
            fullWidth
